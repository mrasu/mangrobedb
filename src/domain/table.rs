...
impl Table {
    ...
    pub fn build_full_path(&self, catalog_file: &File) -> String {
        format!(
            "s3://{}/{}/{}",
            self.schema.location.bucket, self.schema.location.prefix, catalog_file.path
        )
    }

    pub fn build_object_path(&self, path: &str) -> ObjectPath {
        ObjectPath::from(format!("{}/{}", self.schema.location.prefix, path))
    }
}
